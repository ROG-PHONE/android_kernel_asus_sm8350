...
static uint8_t _pt2abyp_st;

enum {
	ABYP_ENTER_NONE = 0,
	ABYP_SYS_POWER_DOWN,
	ABYP_FORCE_ENTER_FAIL,
	ABYP_ENTER_CNT,
};
...
static u32 _iris_read_global_status(void);
...
int iris_lp_abyp_enter(bool blocking)
{
	...
	for (i = 0; i < (frame_interval*2/3 + 1); i++) {
	...
	if (abyp_status_gpio == 0) {
		if ((_iris_read_global_status() & 0x1) == 0x1) {
			iris_send_one_wired_cmd(IRIS_POWER_DOWN_SYS);
			_pt2abyp_st = ABYP_SYS_POWER_DOWN;
		} else {
			iris_send_one_wired_cmd(IRIS_FORCE_ENTER_ANALOG_BYPASS);
			mdelay(frame_interval);
			if ((_iris_read_global_status() & 0x1) == 0x1) {
				iris_send_one_wired_cmd(IRIS_POWER_DOWN_SYS);
				_pt2abyp_st = ABYP_SYS_POWER_DOWN;
			} else
				_pt2abyp_st = ABYP_FORCE_ENTER_FAIL;
		}
	}
	up(&abyp_sem);
	...
	if (debug_on_opt & 0x40)
		abyp_status_gpio = 0;
	if (abyp_status_gpio == 0) {
		if (_pt2abyp_st == ABYP_FORCE_ENTER_FAIL) {
			IRIS_LOGE("FORCE Enter abyp failed!");
...
int iris_lp_abyp_exit(bool blocking)
{
	...
	if (_pt2abyp_st == ABYP_FORCE_ENTER_FAIL) {
		...
		while (cnt--) {
			...
			rc = iris_lightup(pcfg->panel, NULL);
			if (rc != 0) {
	...
	if (_pt2abyp_st == ABYP_SYS_POWER_DOWN) {
	...
	if (abyp_status_gpio == 1) {
		...
		if ((pcfg->lp_ctrl.abyp_lp == 2) || (_pt2abyp_st == ABYP_SYS_POWER_DOWN)) {
...
static u32 _iris_read_global_status(void)
{
	...
	if (pcfg->read_path == PATH_I2C) //use i2c to read
		iris_ioctl_i2c_read(addr, &value);
	else
		value = iris_ocp_read(addr, DSI_CMD_SET_STATE_HS);
